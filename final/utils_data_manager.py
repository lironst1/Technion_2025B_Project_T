...
from __cfg__ import logger, AUTO_CONTRAST, set_props_kw_image, CMAP, DATA_TYPES, CPSAMEvalOut, Stats, \
	CPSAM_EVAL_KW, get_tqdm_kw, DIR_FIGS, CACHE_SIZE, DIR_OUTPUT
from utils import imread, imwrite, pickle_load, pickle_dump, ExcelData, get_image_basenames
...
class DataManager:
	...
	def __init__(self, dir_root, *,
		...
		self.basenames, self.paths = get_image_basenames(dir_root=dir_root, labeled=labeled, sample_size=sample_size)
		...
		if path_excel is None:
			...
			excel_data = ExcelData(path_excel, date=date, view=view)

			max_frame = int(excel_data.final_frame_beta_catenin.max())

			self.basenames = self.basenames[:max_frame + 1]
			...
			self.beta_catenin_intensity = np.repeat(excel_data.beta_catenin_intensity, lengths)
			...
			self.time = dict_(
					...
					limits=np.array([0, (self.num_samples - 1) * time_interval]) / 60,
					units="hours",
			)
	...
	def get_data(self, idx=None, data_type=None):
		...
		if data_type == "image":
			image = imread(self.paths[basename].image, auto_contrast=AUTO_CONTRAST)
			return image

		elif data_type == "labels":
			if self.paths[basename].labels is None:
				return None
			labels = imread(self.paths[basename].labels).round().astype("uint8")
			return labels

		elif data_type == "prob":
			if self.paths[basename].prob is None:
				return None
			prob = pickle_load(self.paths[basename].prob)
			return prob

		elif data_type == "cpsam_out":
			if self.paths[basename].cpsam_out is None:
				return None
			cpsam_out = pickle_load(self.paths[basename].cpsam_out)
			return cpsam_out
	...
	def _save_images(self, idx=None, dir_target=None):
		...
		if dir_target is None:
			dir_target = os.path.join(self.dir_root, DIR_FIGS, "_images")
	...
	def pixel_classifier_predict_prob(self, idx=None, plot=False, **plot_frame_kw):
		...
		filename = os.path.join(self.dir_root, DATA_TYPES.prob.dirname, basename + DATA_TYPES.prob.ext)
		pickle_dump(prob, filename)
	...
	def cpsam_mask(self, idx=None, plot=False, **plot_frame_kw):
		...
		for i, basename in enumerate(basenames):
			...
			# save mask to file
			filename = os.path.join(self.dir_root, DATA_TYPES.cpsam_out.dirname, basename + DATA_TYPES.cpsam_out.ext)
			pickle_dump(cpsam_out, filename)
	...
	def _plot_image(self, idx=None, save_fig=False, imshow_kw=None,
		...
		**set_props_kw : dict, optional
		...
		set_props_kw = dict(
				...
				save_file_name=os.path.join(self.dir_root, DIR_FIGS, "_images", basename) if save_fig else False,
				close_fig=save_fig,
	...
	def plot_frame(self, idx=None, which="default", axs=None, save_fig=False, imshow_kw=None, **set_props_kw):
		...
		which :         str or list[str] or list[list[str]], optional
			...
			- "liora": Liora's graphs (see `plot_liora`), with vertical line at the current image index (time)
			Predefined options are:
			- "default": ["image+mask_bounds", "mask+pred", "stats+liora"]

			For example, if `which=["image", "image+mask_bounds", "mask+pred", "stats", "liora"]`,
		...
		**set_props_kw : dict, optional
		...
		if save_fig:
			save_file_name = os.path.join(self.dir_root, DIR_FIGS, "classification", basename + ".tif")
		...
		for i, w in enumerate(which):
			...
			for plot in w:
				if plot == "image":
					...
					logger.warning("Liora's graphs are not implemented yet. Skipping.")
				else:
	...
	def plot_movie(self, axs=None, save_file_name=None, plot_frame_kw=None, **plot_animation_kw):
		...
		**plot_animation_kw :      sent to imshow
		...
		if save_file_name is None:
			save_file_name = os.path.join(self.dir_root, DIR_FIGS, "classification_movie.gif")
	...
	def plot_stats(self, idx_line=None, ax=None, save_fig=False, **set_props_kw):
		...
		**set_props_kw : dict, optional
		...
		quantities_to_plot = [
			dict_(label=r"Nuclei Count",
					latex=r"$N_t$",
					y=self.stats.count),
			dict_(label=r"Avg. Intensity",
					latex=r"$\frac{\sum_i {I_t^{(i)} \cdot A_t^{(i)}}}{\sum_i {A_t^{(i)}}}$",
					y=self.stats.avg_intensity),
			dict_(label=r"Sum Intensity $\times$ Area",
		...
		for q in quantities_to_plot:
			...
			plot_xy(ax=ax, x=t, y=q.y, xlabel=f"Time [{self.time.unit}]", xlim=self.time.limits)
		...
		if idx_line is not None:
			...
			text = f"Frame: {idx_line}"
			for q in quantities_to_plot:
				text += f"\n{q.label}: {100 * q.y[idx_line]:.1f}%"

			ax.axvline(x=t[idx_line], color="black", linestyle="--", linewidth=1)
			ax.text(x=t[idx_line], y=0.95 * ax.get_ylim()[1],
					s=text,
					fontsize=8,
		...
		if set_props:
			...
			set_props_kw = dict(
					...
					show_fig=not save_fig,
					save_file_name=os.path.join(self.dir_root, DIR_FIGS, "stats") if save_fig else False,
					close_fig=save_fig,
			) | set_props_kw
			Ax.set_props(**set_props_kw)
